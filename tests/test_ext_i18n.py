...
class TestI18nMiddleware(unittest.TestCase):
    ...
    def test_multiple_changes(self):
        class MyHandler(RequestHandler):
            def get(self, **kwargs):
                return BaseResponse(self.request.context['locale'])

        app = Tipfy(rules=[
            Rule('/', endpoint='home', handler=MyHandler)
        ], config={
            'tipfy': {
                'middleware': [I18nMiddleware],
            },
        ...
        self.assertEqual(response.headers['Set-Cookie'].split(';')[0], 'tipfy.locale=en_US')

        response = client.get('/?lang=pt_BR')
        self.assertEqual(response.data, 'pt_BR')
        self.assertEqual(response.headers['Set-Cookie'].split(';')[0], 'tipfy.locale=pt_BR')

        response = client.get('/', headers=[('Cookie', response.headers['Set-Cookie'])])
        self.assertEqual(response.data, 'pt_BR')

        response = client.get('/?lang=en_US', headers=[('Cookie', response.headers['Set-Cookie'])])
        self.assertEqual(response.data, 'en_US')
