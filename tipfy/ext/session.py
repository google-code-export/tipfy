...
#:
#: default_backend
#:     Default session backend when none is specified. Built-in options are
#:     `datastore`, `memcache` or `securecookie`. Default is `securecookie`.
#:
#: secret_key
...
#:
#: cookie_args
#:     Default keyword arguments to set a cookie or securecookie. Keys are:
#:
...
def is_valid_key(key):
    ...
    return _uuid_re.match(key) is not None
...
class SessionStore(object):
    ...
    def _prepare_kwargs(self, kwargs):
        ...
        if max_age and 'expires' not in kwargs:
            kwargs['expires'] = time.time() + max_age

        if session_expires:
            kwargs['session_expires'] = datetime.datetime.fromtimestamp(
                time.time() + session_expires)
...
class DatastoreSession(BaseSession):
    ...
    def get_by_sid(cls, sid):
        ...
        if not entity:
            return cls({}, uuid.uuid4().hex, new=True)
...
class SessionMiddleware(object):
    ...
    def post_dispatch(self, handler, response):
        ...
        if 'session_store' in registry:
            registry['session_store'].save_session(response)
    ...
    def post_dispatch_handler(self, response):
        registry = Tipfy.request.registry
        if 'session_store' in registry:
            registry['session_store'].save_session(response)
